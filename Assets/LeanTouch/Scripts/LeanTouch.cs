...
namespace Lean
{
	...
	public class LeanTouch : MonoBehaviour
	{
		...
		public static void MoveBlockForMeteos(Transform transform, Vector2 Position, Camera camera = null)
		{
			...
			if (camera != null)
			{
				...
				Debug.Log("-- ScreenPosition's Z : " + screenPosition.z + "/");
				screenPosition = (Vector3)Position;
				transform.position = camera.ScreenToWorldPoint(screenPosition);
				
				//transform.position = camera.ScreenToWorldPoint((Vector3)Position);
			}
			/*
			if (transform != null)
			{
				...
				{
					var rectTransform = transform as RectTransform;
					Vector2 newDeltaPos = new Vector2(0, deltaPosition.y);
					
					// If this is RectTransform then modify the anchoredPosition
					if (rectTransform != null)
					{
						rectTransform.anchoredPosition += newDeltaPos;
					}
					// If this is Transform then modify the position
					else
					{
						transform.position = MoveObject(transform.position, newDeltaPos, camera);
					}
				}
			}
			*/
